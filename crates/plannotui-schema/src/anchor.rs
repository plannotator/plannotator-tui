...
//! - everything under `plannotui` is **raw source**: byte offsets into the markdown, the
//!   32-char context on either side, and the document version those offsets belong to.
//!
//! Foreign anchors (written by the web client, MCP tools, older plannotui) must always
...
/// The plannotui-owned part of an anchor. Ignored by the web client, read only by us.
...
impl Anchor {
    ...
    pub fn kind(&self) -> Kind {
        self.plannotui.as_ref().map_or(Kind::Comment, |e| e.kind)
    }
}
...
mod tests {
    ...
    fn serializes_to_the_wire_shape() {
        ...
        assert_eq!(json["plannotui"]["kind"], "looks_good");
        assert_eq!(json["plannotui"]["quote"], "login page");
        assert_eq!(json["plannotui"]["source"]["start"], 11);
        assert_eq!(json["plannotui"]["prefix"], "Ship the **");
        assert_eq!(json["plannotui"]["suffix"], "** by Friday.");
