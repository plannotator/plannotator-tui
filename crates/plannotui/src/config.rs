...
//!
//! `$PLANNOTUI_CONFIG` → `$XDG_CONFIG_HOME/plannotui/config.toml` →
//! `~/.config/plannotui/config.toml`. A missing file means defaults. An unknown key is an
...
/// How plannotui opens inside Herdr.
...
pub(crate) fn config_path(env: impl Fn(&str) -> Option<String>, home: &Path) -> PathBuf {
    if let Some(explicit) = env("PLANNOTUI_CONFIG").filter(|s| !s.is_empty()) {
        return PathBuf::from(explicit);
    }
    let xdg = env("XDG_CONFIG_HOME").map(PathBuf::from).filter(|p| p.is_absolute());
    xdg.unwrap_or_else(|| home.join(".config")).join("plannotui").join("config.toml")
}
...
impl Config {
    ...
    /// The effective config as TOML, for `plannotui config`.
...
mod tests {
    ...
    fn config_path_precedence() {
        ...
        assert_eq!(
            config_path(lookup(&[("PLANNOTUI_CONFIG", "/etc/p.toml"), ("XDG_CONFIG_HOME", "/x")]), home),
            PathBuf::from("/etc/p.toml")
        );
        assert_eq!(
            config_path(lookup(&[("XDG_CONFIG_HOME", "/x")]), home),
            PathBuf::from("/x/plannotui/config.toml")
        );
        // A relative XDG_CONFIG_HOME is ignored, as the spec requires.
        assert_eq!(
            config_path(lookup(&[("XDG_CONFIG_HOME", "rel")]), home),
            PathBuf::from("/home/u/.config/plannotui/config.toml")
        );
        assert_eq!(config_path(lookup(&[]), home), PathBuf::from("/home/u/.config/plannotui/config.toml"));
    }
