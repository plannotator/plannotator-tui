...
fn temp_folder(tag: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("plannotui-launch-{tag}-{}", std::process::id()));
...
fn human_keybind_opens_the_focused_folder_and_delivers_to_its_agent() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            "--env",
            &format!("PLANNOTUI_FILE={}", root.display()),
            "--env",
...
fn agent_skill_delivers_to_and_splits_beside_the_calling_pane() {
    ...
    assert!(args.iter().any(|a| a == "PLANNOTUI_DELIVER_TO=w2:p4"));
    assert!(!args.iter().any(|a| a.starts_with("PLANNOTUI_DELIVER_AGENT")));
    std::fs::remove_dir_all(&root).expect("cleanup");
}
