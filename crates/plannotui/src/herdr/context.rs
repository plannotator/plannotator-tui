...
/// Everything plannotui reads from its environment when Herdr is involved.
...
pub(crate) struct HerdrEnv {
    ...
    /// `PLANNOTUI_FILE`, `PLANNOTUI_DELIVER_TO`, `PLANNOTUI_DELIVER_AGENT`,
    /// `PLANNOTUI_PLACEMENT`: set by the launcher or an agent.
...
impl HerdrEnv {
    ...
    pub(crate) fn from_lookup(env: impl Fn(&str) -> Option<String>) -> Self {
        ...
        Self {
            ...
            context,
            file: non_empty("PLANNOTUI_FILE").map(PathBuf::from),
...
mod tests {
    ...
    fn explicit_deliver_to_wins_and_carries_the_agent_label() {
        let env = env(&[
            ("HERDR_ENV", "1"),
            ("PLANNOTUI_DELIVER_TO", "w1:p1"),
            ("PLANNOTUI_DELIVER_AGENT", "claude"),
